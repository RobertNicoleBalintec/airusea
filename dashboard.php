...
<body>
    ...
    <script>
        ...
        function addStatusColumn() {
            if (confirm('This will add a "status" column to your rentals table. This is required to track cancelled rentals properly. Continue?')) {
        ...
        // Create update_database.php file content (for you to save)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_status_column') {
    try {
        ...
        $sql = "ALTER TABLE rentals ADD COLUMN status VARCHAR(20) DEFAULT NULL";
        $pdo->exec($sql);
        
        // Add an index for better performance
        $pdo->exec("CREATE INDEX idx_status ON rentals(status)");
        
        echo "Database updated successfully! Status column added to rentals table.";
        echo '<br><a href="dashboard.php">Return to Dashboard</a>';
        
        // Log the action
        logEvent($_SESSION['Email'], 'Added status column to rentals table');
...
?>`;
        ...
        // Log the update code for reference
        console.log('Save the following as update_database.php:');
        console.log(updateCode);
    </script>
</body>
</html>

setup_cancellation.php
